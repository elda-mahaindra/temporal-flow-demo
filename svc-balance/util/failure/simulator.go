...
func (s *Simulator) SimulateFailure(ctx context.Context, operation string, accountID string, rules []Rule) error {
	...
	for _, rule := range rules {
		if s.shouldApplyRule(rule, operation, accountID) {
			...
			}).Warn("🚨 Injecting simulated failure for Temporal testing")
...
func (s *Simulator) executeFailure(ctx context.Context, rule Rule) error {
	...
	case "error":
		...
		if message == "" {
			message = fmt.Sprintf("Learning demo failure: %s", rule.Name)
		}
		return fmt.Errorf("DEMO_FAILURE: %s", message)
	...
	case "timeout":
		...
		s.logger.WithField("timeout", timeoutDuration).Debug("⏱️ Simulating timeout for Temporal testing")

		select {
		case <-time.After(timeoutDuration):
			return fmt.Errorf("DEMO_TIMEOUT: operation timed out after %v", timeoutDuration)
		case <-ctx.Done():
			return ctx.Err()
		}
	...
	case "slow":
		...
		s.logger.WithField("delay", delay).Debug("🐌 Simulating slow operation for Temporal testing")
	...
	case "panic":
		message := rule.Message
		if message == "" {
			message = fmt.Sprintf("Learning demo panic: %s", rule.Name)
		}
		panic(fmt.Sprintf("DEMO_PANIC: %s", message))

	default:
		return fmt.Errorf("DEMO_FAILURE: unknown failure type: %s", rule.Type)
	}
}
...
func (s *Simulator) Reset() {
	...
	s.logger.Info("🔄 Failure simulator state reset for new learning session")
}
